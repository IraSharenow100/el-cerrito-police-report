...
\newpage
```{r export_for_Tableau}

# --- Export for Tableau (police) ---  # added
police_tableau <- df_filtered_clean_updated %>%
  dplyr::transmute(
    EVENT_NUMBER   = event_number,              # ID
    INCIDENT_DATE  = received_date,             # choose your date field
    INCIDENT_TYPE  = call_for_service,          # "type"
    LATITUDE       = as.numeric(latitude),
    LONGITUDE      = as.numeric(longitude),
    BAD_LATLON_FLAG = BAD_LATLON_FLAG           # keep temporarily so we can filter
  ) %>%
  # parse date robustly (handles date or datetime)
  dplyr::mutate(
    INCIDENT_DATE = suppressWarnings(lubridate::ymd_hms(INCIDENT_DATE)),
    INCIDENT_DATE = ifelse(is.na(INCIDENT_DATE),
                           as.Date(suppressWarnings(lubridate::ymd(INCIDENT_DATE))),
                           as.Date(INCIDENT_DATE)),
    INCIDENT_DATE = as.Date(INCIDENT_DATE),
    INCIDENT_YEAR  = lubridate::year(INCIDENT_DATE),
    INCIDENT_MONTH = lubridate::month(INCIDENT_DATE, label = TRUE, abbr = TRUE)
  ) %>%
  # drop unusable coords
  dplyr::filter(!is.na(LATITUDE), !is.na(LONGITUDE)) %>%
  # drop bad lat/lon rows if flagged (works for 1/TRUE/"TRUE"/"Y")
  dplyr::filter(!(tolower(as.character(BAD_LATLON_FLAG)) %in% c("1","true","y","yes"))) %>%
  dplyr::select(-BAD_LATLON_FLAG)

readr::write_csv(
  police_tableau,
  "D:/Documents/Employment/2025 job search/Project 2025/el-cerrito-police-report/data/el_cerrito_police_tableau.csv"
)

cat("✅ Exported", nrow(police_tableau), "rows for Tableau.\n")
